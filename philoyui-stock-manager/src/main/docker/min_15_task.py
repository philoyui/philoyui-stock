...
def truncate_tables():
    truncate_macd_sql = "delete from macd_data_entity where interval_type = '15min'"
...
for stock_code in stock_df["code"]:
    ...
    day_data_list = []
    while (min_15_result.error_code == '0') & min_15_result.next():
        day_data_list.append(min_15_result.get_row_data())
    day_data_frame = pd.DataFrame(day_data_list, columns=min_15_result.fields)
