...
def truncate_tables():
    truncate_macd_sql = "delete from macd_data_entity where interval_type = '30min'"
...
for stock_code in stock_df["code"]:
    ...
    startDay = (datetime.datetime.now() - datetime.timedelta(days=9)).strftime("%Y-%m-%d")

    interval_type_string = "Min30"

    min_30_result = bs.query_history_k_data_plus(stock_code, "date,code,open,high,low,close,"
                                                          "volume,amount,adjustflag",
                                                 start_date=startDay, end_date='',
                                                 frequency="30", adjustflag="2")

    day_data_list = []
    while (min_30_result.error_code == '0') & min_30_result.next():
        day_data_list.append(min_30_result.get_row_data())
    day_data_frame = pd.DataFrame(day_data_list, columns=min_30_result.fields)
