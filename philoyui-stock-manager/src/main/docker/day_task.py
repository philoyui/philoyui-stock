...
def mark_tag_stock(symbol, tag_name):
    tag_sql = "INSERT INTO stock_tag (created_time, symbol, tag_name, day_string, interval_type, last_index" \
          ") VALUES (str_to_date('" + day_array[-1 - i] + "', '%%Y-%%m-%%d') , '" \
          + symbol + "', '" + tag_name + "', '" + day_array[-1 - i] + "', 0, -1)"
...
for stock_code in stock_df["code"]:
    ...
    for i in range(len(close_array)-2):
        ...
        if cci_array[-1-i] is not None:
            if cci_array[-1-i] > 100 and cci_array[-2-i] < 100:
                mark_cci_value(symbol_string, interval_type_string, "BREAK_100", -1-i)
            if cci_array[-1-i] < 100 and cci_array[-2-i] > 100:
                mark_cci_value(symbol_string, interval_type_string, "FALL_100", -1-i)
            if cci_array[-1-i] < -100 < cci_array[-2-i]:
                mark_cci_value(symbol_string, interval_type_string, "FALL_NEGATIVE_100", -1-i)
            if cci_array[-2-i] < -100 < cci_array[-1-i]:
                mark_cci_value(symbol_string, interval_type_string, "BREAK_NEGATIVE_100", -1-i)
        ...
        if k.values[length - 1 - i][0] <= 20 and d.values[length - 1 - i][0] < 28:
            mark_kjd_value(symbol_string, interval_type_string, "OVER_SELL", -1 - i)

        if k.values[length - 1 - i][0] >= 80 and d.values[length - 1 - i][0] > 72:
            mark_kjd_value(symbol_string, interval_type_string, "OVER_BUY", -1 - i)
    ...
    try:
        if 3.0 < float(turn_array[-1 - i]) <= 10.0:
            mark_tag_stock(symbol_string, "换手活跃")

        if float(turn_array[-1 - i]) > 10.0:
            mark_tag_stock(symbol_string, "庄控股（大换手）")
    except ValueError:
        print(turn_array[-1 - i])

    try:
        if 0.0 < float(pbMRQ_array[-1 - i]) <= 1.0:
            mark_tag_stock(symbol_string, "破净（周期资源熊市，低估）")
    except ValueError:
        print(pbMRQ_array[-1 - i])

    try:
        if float(peTTM_array[-1 - i]) <= 0.0:
            mark_tag_stock(symbol_string, "市盈率为负不盈利（非周期牛市）")
        if 0.0 < float(peTTM_array[-1 - i]) <= 13.0:
            mark_tag_stock(symbol_string, "低市盈率（非周期牛市）")
        if 21.0 < float(peTTM_array[-1 - i]) <= 28.0:
            mark_tag_stock(symbol_string, "高市盈率（非周期牛市）")
        if float(peTTM_array[-1 - i]) > 28.0:
            mark_tag_stock(symbol_string, "市盈率泡沫（非周期牛市）")
    except ValueError:
        print(peTTM_array[-1 - i])

    try:
        if float(psTTM_array[-1 - i]) <= 0.75:
            mark_tag_stock(symbol_string, "市销率（主营业务未来有竞争力）")
        if float(psTTM_array[-1 - i]) > 10:
            mark_tag_stock(symbol_string, "市销率风险")
    except ValueError:
        print(psTTM_array[-1 - i])
