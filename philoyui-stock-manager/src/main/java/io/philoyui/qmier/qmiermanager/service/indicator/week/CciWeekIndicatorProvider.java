...
@Component
public class CciWeekIndicatorProvider implements IndicatorProvider {
    ...
    public List<TagStockEntity> processTags(StockEntity stockEntity) {
        ...
        for (CciDataEntity cciDataEntity : cciDataEntities) {
            switch (cciDataEntity.getCciType()){
                case BREAK_100:
                    tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI进入强势(周)",cciDataEntity.getDay(), cciDataEntity.getIntervalType(), cciDataEntity.getLastIndex()));
                    break;
                case BREAK_NEGATIVE_100:
                    tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI多头开启(周)",cciDataEntity.getDay(), cciDataEntity.getIntervalType(), cciDataEntity.getLastIndex()));
                    break;
                case FALL_100:
                    tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI空头开启(周)",cciDataEntity.getDay(), cciDataEntity.getIntervalType(), cciDataEntity.getLastIndex()));
                    break;
                case FALL_NEGATIVE_100:
                    tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI进入弱势(周)",cciDataEntity.getDay(), cciDataEntity.getIntervalType(), cciDataEntity.getLastIndex()));
                    break;
            }
        }
        ...
        for (int i = 0; i < topDataList.size()-1; i++) {
            ...
            if(newCciDataEntity.getCciValue() < oldCciDataEntity.getCciValue() && newCciDataEntity.getCloseValue() > oldCciDataEntity.getCloseValue()){
                tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI顶背离(周)",newCciDataEntity.getDay(),newCciDataEntity.getIntervalType(),newCciDataEntity.getLastIndex()));
            }
        }
        ...
        for (int i = 0; i < bottomDataList.size()-1; i++) {
            ...
            if(newCciDataEntity.getCciValue() > oldCciDataEntity.getCciValue() && newCciDataEntity.getCloseValue() < oldCciDataEntity.getCloseValue()){
                tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI底背离(周)",newCciDataEntity.getDay(),newCciDataEntity.getIntervalType(),newCciDataEntity.getLastIndex()));
            }
        }
