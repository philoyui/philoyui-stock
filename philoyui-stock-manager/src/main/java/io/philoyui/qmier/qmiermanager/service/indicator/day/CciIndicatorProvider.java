...
public class CciIndicatorProvider implements IndicatorProvider {
    ...
    public List<TagStockEntity> processTags(StockEntity stockEntity) {
        ...
        for (int i = 0; i < topDataList.size()-1; i++) {
            ...
            if(newCciDataEntity.getCciValue() < oldCciDataEntity.getCciValue() && newCciDataEntity.getCloseValue() > oldCciDataEntity.getCloseValue()){
                tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI顶背离",newCciDataEntity.getDay()));
            }
        }
        ...
        for (int i = 0; i < bottomDataList.size()-1; i++) {
            ...
            if(newCciDataEntity.getCciValue() > oldCciDataEntity.getCciValue() && newCciDataEntity.getCloseValue() < oldCciDataEntity.getCloseValue()){
                tagStockEntities.add(tagStockService.tagStock(stockEntity.getSymbol(),"CCI底背离",newCciDataEntity.getDay()));
            }
        }
    ...
    public void cleanOldData() {
        ...
        tagStockService.deleteByTagName("CCI进入强势");
        tagStockService.deleteByTagName("CCI多头开启");
        tagStockService.deleteByTagName("CCI空头开启");
        tagStockService.deleteByTagName("CCI进入弱势");
        tagStockService.deleteByTagName("CCI结束强势");
        tagStockService.deleteByTagName("CCI结束弱势");
        tagStockService.deleteByTagName("CCI顶背离");
        tagStockService.deleteByTagName("CCI底背离");
